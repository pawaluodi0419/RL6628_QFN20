...
void XGPIO_dut0_1_Intrq_Handle(){
	 ...
	 */
	for(i=0;i<10;i++){
		if(XGpio_Dutx_ReadBit(XPAR_AXI_GPIO_dut0_1_BASEADDR,0,21) == 1){
			FT2Start_Cnt++;
		}
	}
	...
	if(FT2StartHSK == 1)
	{
		if(dut0.g_start00_pulsh_count < 3)
		{
	...
	if((dut0.g_FT2StartSignal==1) && (dut0.g_uartPatternNum == 0x0f))
	{

		dut0.g_dut_start_ready=0x11;
	}

	else if((dut0.g_FT2StartSignal==1) && (dut0.g_uartPatternNum == 0x0e))
	{
		dut0.g_dut_start_ready=0x11;
	}
	...
	else if((dut0.g_FT2StartSignal==1) && (dut0.g_uartPatternNum > 0) && (dut0.g_uartPatternNum <0x0e))
	{
		if(dut0.g_result_fail != 0x00)
		{

				dut0.g_dut_start_ready=0x11; //fail time enough,wait for next test
				dut0.g_result_fail = 0;
